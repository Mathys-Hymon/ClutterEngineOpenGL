...
void LevelMap::Load()
{
	...
	floorMesh = AddActor < clt::CubeActor>("wall", false);

	floorMesh->AddComponent<clt::SoundComponent>("inGame", true, true);

	floorMesh->SetActorLocation({ 0,0,1 });
}

void LevelMap::Update()
{
	floorMesh->AddActorLocationOffset({ 0.001f,0,0});
}
