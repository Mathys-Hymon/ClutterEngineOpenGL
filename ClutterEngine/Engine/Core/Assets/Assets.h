...
namespace clt
{
	...
	public:
		...
		std::weak_ptr<IMaterial> CreateMaterial(const std::string& pName, std::vector<std::weak_ptr<Shader>> pShaders);

		std::weak_ptr<IMaterial> CreateMaterialInstance(const std::string& pName, std::weak_ptr<Material> pParent);
