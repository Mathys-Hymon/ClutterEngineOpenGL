...
void EditorController::Movement(Vector2 pDirection)
{
	...
	Vector3 forward = pDirection.y * mOwner->GetActorTransform().Forward() * dt;
	Vector3 right = pDirection.x * mOwner->GetActorTransform().Right() * dt;
...
void EditorController::MoveVertically(float pDirection)
{
	...
	Vector3 up = pDirection * mOwner->GetActorTransform().Up() * Timer::deltaTime;
...
{ 
	...
	Quaternion actorRot = mOwner->GetActorRotation();
